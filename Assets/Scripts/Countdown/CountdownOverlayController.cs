...
[RequireComponent(typeof(CountdownOverlayRaycast))]
public class CountdownOverlayController : MonoBehaviour
{
    ...
    [SerializeField] private bool enableLeftStickAtStart;
    ...
    private CountdownOverlayRaycast _raycastOverlay;
    private int currentStep = 0;

    private void Awake()
    {
        _raycastOverlay = GetComponent<CountdownOverlayRaycast>();
        _mat = _raycastOverlay.material = new Material(_raycastOverlay.material);
       
        ClearHole();

        if(enableLeftStickAtStart)
        {
            UncoverLeftStick();
        }
    }
